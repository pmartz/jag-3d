...
void DrawNode::execute( DrawInfo& drawInfo )
{
    ...
    BOOST_FOREACH( const jag::draw::DrawNode::CallbackPtr& cb, callbacks )
    {
        if( !( (*cb)( *this, drawInfo ) ) )
        {
            return;
...
void DrawNode::setMaxContexts( const unsigned int numContexts )
{
    if( _commands == NULL )
    {
        JAG3D_ERROR( "setMaxContexts(): jag::draw::DrawNode must have non-NULL _commands." );
    }
    else
    {
...
void DrawNode::deleteID( const jag::draw::jagDrawContextID contextID )
{
...
void DrawNode::setBoundUniforms( DrawInfo& drawInfo )
{
    ...
    jag::draw::ProgramPtr prog( boost::dynamic_pointer_cast< Program >( current[ Command::Program_t ] ) );
