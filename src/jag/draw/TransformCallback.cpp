...
bool TransformCallback::operator()( jag::draw::DrawNode& node, jag::draw::DrawInfo& drawInfo )
{
    ...
    while( flags != 0 )
    {
        if( ( flags & matrixBit ) != 0 )
        {
            ...
            jag::draw::UniformPtr& uniform( drawInfo.getOrCreateUniform( _nameMap[ matrixBit ] ) );
